...
<template>
  <section v-if="statistics" class="area-stat-summary" :aria-label="t('statisticsOverview')">
    <div class="summary-metric">
      ...
      <div class="summary-bar summary-bar--employment" aria-hidden="true">
        <span class="summary-bar__segment summary-bar__segment--employed" :style="{ width: `${statistics.employedShare2013}%` }" />
        <span class="summary-bar__segment summary-bar__segment--unemployed" :style="{ width: `${statistics.unemployment2013}%` }" />
      </div>
    ...
    <div class="summary-metric">
      ...
      <div class="student-pictogram" :aria-label="`${t('students')}: ${formatPercent(statistics.studentShare2013)}`">
        <span v-for="index in studentIcons" :key="index" class="student-icon">
          <svg class="student-icon__base" viewBox="0 0 32 40" aria-hidden="true"><path d="M16 2 2 8.3l14 6.3 11-4.9v6.7h2V8.3L16 2Zm-7.6 10.4v5.1c0 3.2 3.4 5.8 7.6 5.8s7.6-2.6 7.6-5.8v-5.1L16 15.8l-7.6-3.4ZM16 24.7c-6.5 0-11.2 3.7-11.2 8.9V38h22.4v-4.4c0-5.2-4.7-8.9-11.2-8.9Z" /></svg>
          <span class="student-icon__fill" :style="{ width: studentIconFill(index) }"><svg viewBox="0 0 32 40" aria-hidden="true"><path d="M16 2 2 8.3l14 6.3 11-4.9v6.7h2V8.3L16 2Zm-7.6 10.4v5.1c0 3.2 3.4 5.8 7.6 5.8s7.6-2.6 7.6-5.8v-5.1L16 15.8l-7.6-3.4ZM16 24.7c-6.5 0-11.2 3.7-11.2 8.9V38h22.4v-4.4c0-5.2-4.7-8.9-11.2-8.9Z" /></svg></span>
        </span>
      </div>
    </div>
    ...
    <div v-if="displayedPopulation !== null" class="summary-population">
      ...
      <span v-if="populationChangeText" :class="['population-change', `population-change--${populationChangeDirection}`]">
    ...
    <div class="summary-metric">
      ...
      <div class="summary-bar summary-bar--languages" aria-hidden="true">
        <span class="summary-bar__segment summary-bar__segment--finnish" :style="{ width: `${statistics.language2015.finnish}%` }" />
        <span class="summary-bar__segment summary-bar__segment--swedish" :style="{ width: `${statistics.language2015.swedish}%` }" />
        <span class="summary-bar__segment summary-bar__segment--other-language" :style="{ width: `${statistics.language2015.other}%` }" />
      </div>
  ...
  <p v-else-if="failed" class="area-stat-summary area-stat-summary--empty">{{ t('statisticsUnavailable') }}</p>
</template>
